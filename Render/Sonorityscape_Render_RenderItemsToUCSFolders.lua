...
ucs_root_folder = "F:\\Sounds\\Daniel Petras\\My Sounds\\"
sample_rate = 48000
channels = 2
format = "evaw"
...
function Render_Items(items, ucs_table)
  ...
  for i = 1, #items do
    ...
    local item_prefix = Get_Item_Prefix(items[i])
    
      if item_prefix == nil or Is_Full_UCS_Name(items[i]) == false then
      
        Render_Item(ucs_root_folder, items[i])
        goto continue
    ...
    for k, v in pairs(ucs_table) do
      ...
      if item_prefix == k then
        ...
        Render_Item(ucs_root_folder..v, items[i])
...
function Render_Item(path, item)
